...
class Button(UIComponent):
    ...
    def draw(self, surface):
        color = (self.hover_color if self.hovered 
                 else self.color)
...
class InputBox(UIComponent):
    ...
    @safe_draw
    def draw(self, surface):
        color = (self.color_active if self.active 
                 else self.color_inactive)
        pygame.draw.rect(surface, WHITE, self.rect)
        pygame.draw.rect(surface, color, self.rect, 2, 
                        border_radius=5)

        display_text = (self.text if self.text 
                       else self.placeholder)
        text_color = BLACK if self.text else DARK_GRAY

        text_surface = self.font.render(display_text, 
                                       True, text_color)
...
class BillSplitterApp:
    ...
    def setup_ui(self):
        ...
        self.dish_price_input = InputBox(
            450, 200, 250, 50, "Price (THB)...", 
            numeric_only=True)
        self.person_name_input = InputBox(
            50, 200, 600, 50, "Person name...")

        self.start_button = Button(
            300, 300, 300, 60, "Start Bill Split", GREEN)
        ...
        self.calculate_button = Button(
            300, 600, 300, 60, "Calculate Bills", GREEN)
        self.restart_button = Button(
            300, 600, 300, 60, "Start Over", BLUE)

    def save_results_to_file(self):
        """Save bill summary with timestamp."""
        try:
    ...
    def draw_add_dishes_screen(self):
        ...
        instruction = self.small_font.render(
            "Enter dish name & price", True, DARK_GRAY)
    ...
    def draw_add_people_screen(self):
        ...
        instruction = self.small_font.render(
            "Enter each person's name", True, DARK_GRAY)
        self.screen.blit(instruction, (50, 120))
        ...
        y = 280
        p_header = self.normal_font.render(
            f"People ({len(self.people)}):", True, BLACK)
    ...
    def draw_assign_orders_screen(self):
        ...
        header = self.header_font.render(
            f"What did {current_person.name} eat?", 
            True, BLUE)
        self.screen.blit(header, (50, 50))
        ...
        for dish in list(self.dishes.values()):
            ...
            text = self.normal_font.render(
                check + dish.get_info(), True, BLACK)
            self.screen.blit(text, (60, y + 5))

            y += 50

        is_last = (self.current_person_index >= 
                  len(self.people) - 1)
        self.next_button.text = (
            "Finish" if is_last else "Next Person")
        self.next_button.draw(self.screen)
        self.back_button.draw(self.screen)
    ...
    def draw_results_screen(self):
        ...
        pygame.draw.line(self.screen, DARK_GRAY, 
                        (50, y), (WINDOW_WIDTH - 50, y), 2)
        y += 30

        total_txt = self.header_font.render(
            f"Total: THB {total_bill:.2f}", True, BLUE)
        self.screen.blit(total_txt, (50, y))

        thank = self.normal_font.render(
            "PLEASE PAY NA KRUB", True, RED)
        thank_rect = thank.get_rect(
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if self.state == STATE_MENU:
                ...
                if self.add_person_button.handle_event(
                        event):
                    self.add_person()
